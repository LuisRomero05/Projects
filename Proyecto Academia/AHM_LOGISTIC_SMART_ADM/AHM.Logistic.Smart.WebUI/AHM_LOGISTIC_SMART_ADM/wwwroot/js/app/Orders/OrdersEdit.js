...
function GetOrdersDetailList() {
    ...
    $.ajax({
        type: "GET",
        url: BaseUrl + "/Orders/OrderDetailsList?Id=" + id,
    }).done(function (data, index) {
        data.data.forEach(function (item) {
            Precios.forEach(function (pre) {
                if (item.pro_Id == pre.id && item.ode_Status == 1 && item.sor_Id == id) {
                    Productos.push({ id: pre.id, idDet: item.ode_Id, canti: item.ode_Amount, descripcion: pre.descripcion, precio: pre.precio, total: item.ode_TotalPrice });
                    total += item.ode_Amount * pre.precio;
                }
            });
        });
        ...
        console.log("Productos Base",Productos);
        console.log(Precios);
...
function InsertarOrden() {
    ...
    Swal.fire({
        width: '20%',
        height: '20%',
        text: "¿Estás seguro que deseas guardar el registro?",
        icon: 'info',
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                ...
                UpdateStock();
                iziToast.success({
                    message: '¡La orden se ha actualizado exitosamente!'
                });
                setTimeout(function () {
                    location.assign(BaseUrl + "/orders/index");
                }, 1500)
                InsertarNewOrderDetail();
                EliminarProducto();
            });
        }
    })
...
function EliminarProducto() {
    ...
    DeleteProducts.forEach(function (item) {
        ...
        $.ajax({
            type: "DELETE",
            url: BaseUrl + "/orders/DeleteDetailProduct?Id=" + parseInt(id) + "&proId=" + parseInt(item.IdProduct) + "&Mod=" + idu,
            success: function (response) {
...
//INSERTAR NUEVO PRODUCTO AL DETALLE
function InsertarNewOrderDetail() {
    var InsertProDetail = [];
    ProductosDetail.forEach(function (pre) {
        InsertProDetail.push({
            ...
            "sor_Id": parseInt(id),
            "ode_IdUserCreate": 1,
            "ode_IdUserModified": 1,
        });
    });

    var Insertpro = {
        "sor_details": InsertProDetail,
    };

    $.ajax({
        type: "POST",
        url: BaseUrl + "/orders/InsertOrderDetail",
        data: Insertpro,
        dataType: "json",
    }).done(function (data) {
