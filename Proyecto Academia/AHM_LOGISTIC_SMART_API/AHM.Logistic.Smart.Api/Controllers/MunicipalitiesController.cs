...
namespace AHM.Logistic.Smart.Api.Controllers
{
        ...
        public class MunicipalitiesController : ControllerBase
        {
            ...
            public MunicipalitiesController(CatalogService catalogServices, IMapper mapper)
            {
                _mapper = mapper;
        ...
        {
            var result = _catalogServices.ListMunicipalities();
            return Ok(result);
        }
        ...
        {
            var IdFind = Id;
            var result = _catalogServices.FindMunicipalities(x => x.mun_Id == Id);
            return Ok(result);
        }
        ...
        [HttpPost("Insert")]
            ...
            public IActionResult Insert(MunicipalitiesModel items)
            {
                ...
                var result = _catalogServices.RegisterMunicipalities(item);
            if (!result.Success)
            ...
            public IActionResult Update(int Id, MunicipalitiesModel items)
            {
                var item = _mapper.Map<tbMunicipalities>(items);
                var IdUser = Id;
                var result = _catalogServices.UpdateMunicipalities(IdUser, item);
                return Ok(result);
            }
            ...
            public IActionResult Delete(int Id, int Mod)
            {
                var ModUser = Mod;
                var IdUser = Id;
                var result = _catalogServices.DeleteMunicipalities(IdUser, ModUser);
                return Ok(result);
            }
        }
}
